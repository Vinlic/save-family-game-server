...
async function checkTicket(request) {
  ...
  if (ticket == null) throw new APIException(EX.API_TICKET_EXPIRED);
  if (request.remoteIP && request.remoteIP != ticket.ipAddress) {
    ...
    if (ticket.ipAddressSwitchTimeIntervals.length >= 10) {
      const averageInterval =
        totalInterval / ticket.ipAddressSwitchTimeIntervals.length;
      if (averageInterval < 1800) {
        [...ticket.oldIPAddresses, ticket.ipAddress].forEach((ip) =>
          blockedIPAddresses.push(ip)
        );
    ...
    ticket.ipAddressSwitchTimeIntervals.push(
      util.unixTimestamp() - (ticket.createTime + totalInterval)
    );
  }
  await ticket.save();
  return ticket;
